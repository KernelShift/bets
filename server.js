...
app.post('/admin/api/deposits/create', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  ...
  // ======== DANGEROUS: Generic DB editor for illustration only ========

function adminAssertTableName(name) {
  // prevent injection: only allow alphanum + underscore
  if (!/^[a-zA-Z0-9_]+$/.test(name)) throw new Error('Bad table name');
}
...
function adminGetTableInfo(table) {
  adminAssertTableName(table);
  return db.prepare(`PRAGMA table_info(${table})`).all();
}

function adminPkColumn(info) {
  const pk = info.find(c => c.pk === 1);
  return pk ? pk.name : null;
}

// list tables
app.get('/admin/api/db/tables', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    return res.json({ ok: true, tables: adminGetTables() });
  } catch (e) {
...
// schema for a table
app.get('/admin/api/db/:table/schema', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    const table = String(req.params.table || '');
    const info = adminGetTableInfo(table);
    const pk = adminPkColumn(info);
    return res.json({ ok: true, table, columns: info, pk });
  } catch (e) {
    return res.status(400).json({ ok: false, error: String(e.message || e) });
  }
});
...
// rows for a table
app.get('/admin/api/db/:table/rows', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    const table = String(req.params.table || '');
    adminAssertTableName(table);
    ...
    // stable ordering
    const orderCol = pk || info[0]?.name || 'rowid';

    const rows = db.prepare(`SELECT * FROM ${table} ORDER BY ${orderCol} DESC LIMIT ? OFFSET ?`)
      .all(limit, offset);

    return res.json({ ok: true, table, pk, rows, limit, offset });
  } catch (e) {
    return res.status(400).json({ ok: false, error: String(e.message || e) });
  }
});

// update one cell (raw text) or patch fields
// body: { pkValue, patch: { col: "raw string" } }
app.post('/admin/api/db/:table/update', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    const table = String(req.params.table || '');
    adminAssertTableName(table);
    ...
    const info = adminGetTableInfo(table);
    const pk = adminPkColumn(info);
    if (!pk) return res.status(400).json({ ok: false, error: 'No primary key (pk) column found for this table.' });

    const pkValue = req.body?.pkValue;
    const patch = req.body?.patch || {};
    if (pkValue === undefined || pkValue === null) return res.status(400).json({ ok: false, error: 'Missing pkValue' });

    // Validate column names
    const validCols = new Set(info.map(c => c.name));
    const cols = Object.keys(patch).filter(c => validCols.has(c) && c !== pk);

    if (cols.length === 0) return res.status(400).json({ ok: false, error: 'No valid columns to update.' });

    const sets = cols.map(c => `${c} = ?`).join(', ');
    const values = cols.map(c => patch[c]); // raw string is fine

    const stmt = db.prepare(`UPDATE ${table} SET ${sets} WHERE ${pk} = ?`);
    const r = stmt.run(...values, pkValue);

    return res.json({ ok: true, changed: r.changes });
  } catch (e) {
    return res.status(400).json({ ok: false, error: String(e.message || e) });
  }
});
...
// insert row
// body: { row: { col: "raw string", ... } }
app.post('/admin/api/db/:table/insert', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    const table = String(req.params.table || '');
    adminAssertTableName(table);
    ...
    if (cols.length === 0) return res.status(400).json({ ok: false, error: 'No valid columns provided.' });

    const qs = cols.map(_ => '?').join(',');
    const stmt = db.prepare(`INSERT INTO ${table} (${cols.join(',')}) VALUES (${qs})`);
    const r = stmt.run(...cols.map(c => row[c]));

    return res.json({ ok: true, lastInsertRowid: r.lastInsertRowid });
  } catch (e) {
    return res.status(400).json({ ok: false, error: String(e.message || e) });
  }
});
...
// delete row by pk
// body: { pkValue }
app.post('/admin/api/db/:table/delete', requireAuth, require2faIfEnabled, requireAdminPanel, (req, res) => {
  try {
    const table = String(req.params.table || '');
    adminAssertTableName(table);
    ...
    const info = adminGetTableInfo(table);
    const pk = adminPkColumn(info);
    if (!pk) return res.status(400).json({ ok: false, error: 'No primary key column found.' });

    const pkValue = req.body?.pkValue;
    if (pkValue === undefined || pkValue === null) return res.status(400).json({ ok: false, error: 'Missing pkValue' });

    const stmt = db.prepare(`DELETE FROM ${table} WHERE ${pk} = ?`);
    const r = stmt.run(pkValue);
...
app.get('/main', requireAuth, require2faIfEnabled, async (req, res) => {
  ...
  // My totals (USD cents)
  const myDeposited = db.prepare(`
    SELECT COALESCE(SUM(usd_cents), 0) AS total
    FROM deposits
    WHERE user_id = ? AND status='credited'
  `).get(uid).total;

  const myBetUsdCents = myDeposited * 2;

  // Rate display only (BTC/USD)
  let btcUsd = null;
  try { btcUsd = await getBtcUsdRate(); } catch {}

  // Leaderboard from deposits (USD) + team
  const rows = db.prepare(`
    SELECT
      u.display_name,
      u.username,
      COALESCE(u.team, '') AS team,
      (2 * COALESCE(SUM(d.usd_cents), 0)) AS bet_usd_cents
    FROM users u
    LEFT JOIN deposits d
      ON d.user_id = u.id AND d.status='credited'
    GROUP BY u.id
    HAVING bet_usd_cents > 0
    ORDER BY bet_usd_cents DESC, u.username ASC
  `).all();

  const hasBet = myDeposited > 0;
  ...
  return res.render('main', {
    hasBet,
    myDepositedUsdCents: myDeposited,
    myBetUsdCents,
    btcUsd,        // can be null if fetch fails
    top3,
    others,
    message: null
  });
...
app.get('/settings', requireAuth, require2faIfEnabled, async (req, res) => {
  ...
  const user = db.prepare(`
    SELECT username, display_name, wallet_address, twofa_enabled, COALESCE(team,'') AS team
    FROM users
    WHERE id = ?
  `).get(req.session.userId);
